...
impl PartialEq for Error {
    fn eq(&self, other: &Error) -> bool {
        ...
        match (self, other) {
            ...
            (&StreamHeader { byte: byte1 }, &StreamHeader { byte: byte2 }) => byte1 == byte2,
            (
                &StreamHeaderMismatch { bytes: ref bytes1 },
                &StreamHeaderMismatch { bytes: ref bytes2 },
            ) => bytes1 == bytes2,
            (&UnsupportedChunkType { byte: byte1 }, &UnsupportedChunkType { byte: byte2 }) => {
                byte1 == byte2
            }
            (
                &UnsupportedChunkLength {
...
impl error::Error for Error {
    fn description(&self) -> &str {
        match *self {
            ...
            Error::StreamHeader { .. } => "snappy: corrupt input (missing stream header)",
            Error::StreamHeaderMismatch { .. } => "snappy: corrupt input (stream header mismatch)",
            Error::UnsupportedChunkType { .. } => "snappy: corrupt input (unsupported chunk type)",
